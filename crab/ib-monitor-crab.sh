...
if [ "${UPLOAD_UNIQ_ID}" != "" ]; then
  ...
  if [ "X$TEST_PASSED" = Xfalse ]; then
    ...
    echo 'CRAB_TESTS;ERROR,CRAB Tests'-${CRABCLIENT_TYPE}',See Logs,CRABTests-'${CRABCLIENT_TYPE} >> $WORKSPACE/testsResults/crab-${CRABCLIENT_TYPE}.txt
    CRAB_OK=false
    ...
    echo 'CRAB_TESTS;OK,CRAB Tests'-${CRABCLIENT_TYPE}',See Logs,CRABTests-'${CRABCLIENT_TYPE} >> $WORKSPACE/testsResults/crab-${CRABCLIENT_TYPE}.txt
    CRAB_OK=true
